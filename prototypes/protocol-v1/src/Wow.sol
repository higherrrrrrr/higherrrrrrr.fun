...
contract Wow is IWow, Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable, IERC721Receiver {
    ...
    WowFeatureRegistry public featureRegistry;
    ...
    constructor(
        ...
        featureRegistry = WowFeatureRegistry(_featureRegistry);
    ...
    /// @notice Sets the Feature Registry contract address
    /// @param _featureRegistry The address of the Feature Registry contract
    function setFeatureRegistry(address _featureRegistry) external {
        require(msg.sender == factory, "Not authorized");
        require(_featureRegistry != address(0), "Invalid registry address");
        featureRegistry = WowFeatureRegistry(_featureRegistry);
    }
    ...
    function buy(
        ...
        // Store original function logic result
        uint256 purchaseAmount = _processBuy(
            recipient,
            refundRecipient,
            orderReferrer,
            comment,
        ...
        // Check for conviction minting
        address convictionFeature = featureRegistry.getFeature(keccak256("CONVICTION"));
        if (convictionFeature != address(0)) {
            IConvictionFeature(convictionFeature).checkAndMintConviction(
                recipient,
                purchaseAmount,
                totalSupply(),
                name()
        ...
        if (dynamicNameFeature != address(0)) {
            ...
            IDynamicNameFeature(dynamicNameFeature).checkAndUpdateName(currentPrice);
        }

        return purchaseAmount;
    }

    // Move original buy logic to internal function
    function _processBuy(
        ...
        // Original buy function logic here
        // Cut and paste the original buy function implementation
    }
    ...
    function sell(
        ...
        uint256 saleAmount = _processSell(
            tokensToSell,
            recipient,
            orderReferrer,
            comment,
    ...
    // Move original sell logic to internal function
    function _processSell(
        ...
        // Original sell function logic here
        // Cut and paste the original sell function implementation
    }
